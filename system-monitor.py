...
async def check_main_engine(msg):
    global main_ok, main_fail
    try:
        async with httpx.AsyncClient(timeout=30) as c:
            t0 = time.time()
            r = await c.post(MAIN_ENGINE_EP, headers=HEADERS, json={"message": msg})
...
def run_auto_check():
    ...
    main_result, real_data, main_ms = loop.run_until_complete(check_main_engine(msg))
    ...
    # Build live stats
    main_rate = round(main_ok/(main_ok+main_fail)*100, 1) if (main_ok+main_fail) > 0 else 0
...
{'='*50}
  ...
  SEOBAIKE 主引擎  | 成功: {main_ok} | 失敗: {main_fail} | 成功率: {main_rate}% | 延遲: {main_ms}ms
  {main_result}

  ai-gateway  | 成功: {gw_ok} | 失敗: {gw_fail} | 成功率: {gw_rate}% | 延遲: {gw_ms}ms
  {gw_result}

  {'▓' * int(main_rate/2)}{'░' * (50-int(main_rate/2))} 主引擎 {main_rate}%
...
總節點數: {l1 if l1 != '?' else 0}+{l2 if l2 != '?' else 0}+{l3 if l3 != '?' else 0}+{l4 if l4 != '?' else 0} = {(int(l1) if str(l1).isdigit() else 0)+(int(l2) if str(l2).isdigit() else 0)+(int(l3) if str(l3).isdigit() else 0)+(int(l4) if str(l4).isdigit() else 0)} 個約束節點"""
    ...
    # Add to log
    log_lines.insert(0, f"[{now}] #{round_count} MAIN:{main_ms}ms GW:{gw_ms}ms | msg={msg}")
...
【累計統計】
  SEOBAIKE 主引擎 : {main_ok} 成功 / {main_fail} 失敗 = {main_rate}%
