...
-- RLS（先 DROP 再 CREATE，避免 IF NOT EXISTS 語法不支援）
...
CREATE POLICY "service_role_products" ON products FOR ALL USING (current_setting('role') = 'service_role');
DROP POLICY IF EXISTS "service_role_purchases" ON purchases;
CREATE POLICY "service_role_purchases" ON purchases FOR ALL USING (current_setting('role') = 'service_role');
DROP POLICY IF EXISTS "service_role_purchase_items" ON purchase_items;
CREATE POLICY "service_role_purchase_items" ON purchase_items FOR ALL USING (current_setting('role') = 'service_role');
DROP POLICY IF EXISTS "service_role_reviews" ON reviews;
CREATE POLICY "service_role_reviews" ON reviews FOR ALL USING (current_setting('role') = 'service_role');
...
-- 3. DROP 所有函數再重建（避免參數名衝突）
DROP FUNCTION IF EXISTS get_gpu_utilization();
DROP FUNCTION IF EXISTS call_opus_gpu(TEXT, TEXT, JSONB);
DROP FUNCTION IF EXISTS stress_cloudflare_edge(UUID, TEXT);
DROP FUNCTION IF EXISTS get_user_recommendations_insane_v6(UUID);

-- 4. get_gpu_utilization
CREATE FUNCTION get_gpu_utilization()
RETURNS FLOAT LANGUAGE plpgsql AS $$
DECLARE latest FLOAT;
BEGIN
...
-- 5. call_opus_gpu（含 token log 自我監控）
CREATE FUNCTION call_opus_gpu(
    p_user_id TEXT,
...
BEGIN
    ...
    BEGIN v_gpu_util := get_gpu_utilization(); EXCEPTION WHEN OTHERS THEN v_gpu_util := 0; END;
    ...
    RETURN jsonb_build_object(
        'model', p_model, 'user_id', p_user_id, 'gpu_utilization', v_gpu_util,
        'tokens_used', v_tokens_used, 'execution_time_ms', round((v_exec_time * 1000)::numeric, 2), 'timestamp', now()
    );
EXCEPTION WHEN OTHERS THEN
    INSERT INTO ai_token_log (username, tokens_used, model, execution_time, response_status, error_message)
    VALUES (current_user, COALESCE(v_tokens_used, 0), p_model, extract(epoch from clock_timestamp()) - v_start, 'failed', SQLERRM);
    RETURN jsonb_build_object('error', SQLERRM, 'model', p_model, 'status', 'failed');
END;
$$;

-- 6. stress_cloudflare_edge
CREATE FUNCTION stress_cloudflare_edge(p_user_id UUID, p_user_email TEXT)
RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER AS $$
BEGIN
    RETURN jsonb_build_object(
        'edge_locations', ARRAY['NRT','LAX','LHR','FRA','SYD','GRU','HKG','IAD','AMS','SIN'],
        'user_id', p_user_id, 'user_email', p_user_email, 'status', 'ok', 'timestamp', now()
    );
END;
$$;

-- 7. 終極暴力函數 v6
CREATE FUNCTION get_user_recommendations_insane_v6(p_uid UUID)
...
BEGIN
    ...
    SELECT email, role INTO user_email, user_role FROM auth.users WHERE id = p_uid;

    <<query_loop>>
    LOOP
        WITH
        user_info AS (SELECT * FROM auth.users WHERE id = p_uid),
        user_purchases AS (
            SELECT p.*, row_number() OVER (ORDER BY p.created_at DESC) as rn
            FROM purchases p WHERE p.user_id = p_uid
        ),
        ...
        final_recs AS (
            SELECT jsonb_agg(jsonb_build_object(
                'product_id', ps.id, 'name', ps.name, 'category', ps.category,
                'score', ps.combined_score, 'price', ps.price
            )) as recs FROM product_scores ps
        )
        SELECT jsonb_build_object(
            'user', row_to_json(ui),
            'purchase_stats', (SELECT jsonb_agg(row_to_json(ca)) FROM category_affinity ca),
            'recommendations', (SELECT recs FROM final_recs),
            'retry', retry_count
        ) INTO user_data FROM user_info ui;
    ...
    BEGIN
        SELECT call_opus_gpu(p_uid::TEXT,
            CASE WHEN retry_count > 0 THEN 'opus-4.6-max-retry' ELSE 'opus-4.6-max' END,
            jsonb_build_object('user_email', user_email, 'retry', retry_count)
        ) INTO opus_result;
    EXCEPTION WHEN OTHERS THEN
    ...
    BEGIN
        cf_result := stress_cloudflare_edge(p_uid, user_email);
    EXCEPTION WHEN OTHERS THEN
        cf_result := jsonb_build_object('cf_error', SQLERRM);
    END;

    result := result || jsonb_build_object(
        'opus_ai', opus_result, 'cloudflare_stress', cf_result, 'generated_at', now()
    );
    ...
    INSERT INTO patent_evidence_log (
        ...
        current_user, 'get_user_recommendations_insane_v6', gpu_util_val, 'opus-4.6-max',
        ARRAY['NRT','LAX','LHR','FRA','SYD','GRU','HKG','IAD','AMS','SIN'],
        total_exec_time,
        current_setting('max_parallel_workers_per_gather')::INT,
        current_setting('work_mem'),
        'v6_insane_retry_' || retry_count::text
    );
...
DECLARE
    ...
    v_t TEXT; v_f TEXT; v_ok INT := 0; v_total INT;
BEGIN
    v_total := array_length(v_tables, 1) + array_length(v_funcs, 1);
    FOREACH v_t IN ARRAY v_tables LOOP
        IF EXISTS (SELECT 1 FROM pg_tables WHERE tablename = v_t) THEN v_ok := v_ok + 1;
        ELSE RAISE NOTICE 'MISSING TABLE: %', v_t; END IF;
    END LOOP;
    FOREACH v_f IN ARRAY v_funcs LOOP
        IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = v_f) THEN v_ok := v_ok + 1;
        ELSE RAISE NOTICE 'MISSING FUNC: %', v_f; END IF;
    END LOOP;
    RAISE NOTICE '=== Migration 048: %/% OK ===', v_ok, v_total;
END;
$$;
